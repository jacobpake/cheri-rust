...
impl CheriotCapImportParser {
    fn convert(cx: &mut AcceptContext<'_, '_>, args: &ArgParser) -> Option<AttributeKind> {
        ...
        let (import_kind, attr_name) = if cx.attr_path.segments.contains(&sym::cheriot_mmio) {
            (rustc_attr_ir::CheriotCapImportKind::MMIO, sym::cheriot_mmio.to_string())
        } else if cx.attr_path.segments.contains(&sym::cheriot_shared_object) {
            (
                rustc_attr_ir::CheriotCapImportKind::SharedObject,
        ...
        Some(AttributeKind::CheriotCapImport(rustc_attr_ir::CheriotCapImportAttr {
            import_kind,
            name,
            name_span,
            permissions,
