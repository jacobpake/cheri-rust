...
fn test_format_f64() {
    ...
    assert_eq!("-1e16", format!("{:?}", -high_cutoff));
    // assert!(!is_exponential(&format!(
    //     "{:?}",
    //     high_cutoff * (1.0 - 2.0 * f64::EPSILON)
    // )));
...
#[no_mangle]
extern "C" fn test_messy() -> i32 {
    test_format_f64();
    test_format_f32();
    test_format_f64_rounds_ties_to_even();
    test_format_f32_rounds_ties_to_even();
    test_format_f64_max_precision_exponential();
    0
}
