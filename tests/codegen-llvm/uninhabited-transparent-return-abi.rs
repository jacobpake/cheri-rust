...
pub fn test_uninhabited_ret_by_ref() {
    ...
    // CHECK-NEXT: call[[ADDRSPACE]] void @llvm.lifetime.start.p{{[0-9]+}}({{(i64 24, )?}}ptr[[ADDRSPACE]] nonnull %_1)
    // CHECK-NEXT: call[[ADDRSPACE]] void @opaque({{.*}} sret([24 x i8]) {{.*}} %_1) #2
...
pub fn test_uninhabited_ret_by_ref_with_arg(rsi: u32) {
    ...
    // CHECK-NEXT: call[[ADDRSPACE]] void @llvm.lifetime.start.p{{[0-9]+}}({{(i64 24, )?}}ptr[[ADDRSPACE]] nonnull %_2)
    // CHECK-NEXT: call[[ADDRSPACE]] void @opaque_with_arg({{.*}} sret([24 x i8]) {{.*}} %_2, i32 noundef{{( signext)?}} %rsi) #2
    // CHECK-NEXT: unreachable
    unsafe {
        opaque_with_arg(rsi);
    }
