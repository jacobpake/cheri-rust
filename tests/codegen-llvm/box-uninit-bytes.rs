...
// CHECK: declare {{(dso_local )?}}noalias noundef ptr[[ADDRSPACE]]   @{{.*}}__rust_alloc(i{{[0-9]+}} noundef{{( signext)?}}, i{{[0-9]+}} allocalign noundef{{( signext)?}}) unnamed_addr[[ADDRSPACE]] [[RUST_ALLOC_ATTRS:#[0-9]+]]
