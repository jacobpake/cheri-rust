...
fn index_on_struct(a: &[Foo], index: usize) -> &Foo {
    // CHECK: getelementptr inbounds{{( nuw)?}} {{%Foo|\[8 x i8\]}}, ptr[[ADDRSPACE]] %a.0, {{i64|i32}} %index
    &a[index]
}
...
fn offset_on_struct(a: *const Foo, index: usize) -> *const Foo {
    // CHECK: getelementptr inbounds{{( nuw)?}} {{%Foo|\[8 x i8\]}}, ptr[[ADDRSPACE]] %a, {{i64|i32}} %index
    unsafe { a.add(index) }
}
...
fn index_on_i32(a: &[i32], index: usize) -> &i32 {
    // CHECK: getelementptr inbounds{{( nuw)?}} {{i32|\[4 x i8\]}}, ptr[[ADDRSPACE]] %a.0, {{i64|i32}} %index
    &a[index]
}
...
fn offset_on_i32(a: *const i32, index: usize) -> *const i32 {
    // CHECK: getelementptr inbounds{{( nuw)?}} {{i32|\[4 x i8\]}}, ptr[[ADDRSPACE]] %a, {{i64|i32}} %index
    unsafe { a.add(index) }
}
