...
pub unsafe fn ctlz_u16(x: u16) -> u32 {
    // CHECK: %[[tmp:.*]] = call[[ADDRSPACE]] i16 @llvm.ctlz.i16(i16 %x, i1 false)
...
pub unsafe fn ctlz_nzu16(x: u16) -> u32 {
    // CHECK: %[[tmp:.*]] = call[[ADDRSPACE]] i16 @llvm.ctlz.i16(i16 %x, i1 true)
    // CHECK: zext i16 %[[tmp]] to i32
    ctlz_nonzero(x)
}
...
pub unsafe fn ctlz_u32(x: u32) -> u32 {
    // CHECK: call[[ADDRSPACE]] i32 @llvm.ctlz.i32(i32 %x, i1 false)
    // CHECK-NOT: zext
    // CHECK-NOT: trunc
    ctlz(x)
}
...
pub unsafe fn ctlz_nzu32(x: u32) -> u32 {
    // CHECK: call[[ADDRSPACE]] i32 @llvm.ctlz.i32(i32 %x, i1 true)
    // CHECK-NOT: zext
    // CHECK-NOT: trunc
    ctlz_nonzero(x)
}
...
pub unsafe fn ctlz_u64(x: u64) -> u32 {
    // CHECK: %[[tmp:.*]] = call[[ADDRSPACE]] i64 @llvm.ctlz.i64(i64 %x, i1 false)
...
pub unsafe fn ctlz_nzu64(x: u64) -> u32 {
    // CHECK: %[[tmp:.*]] = call[[ADDRSPACE]] i64 @llvm.ctlz.i64(i64 %x, i1 true)
    // CHECK: trunc i64 %[[tmp]] to i32
    ctlz_nonzero(x)
}
